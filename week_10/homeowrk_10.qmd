...
> *"Is `distance` still useful in the model once `dep_delay` and `air_time` are already included?"*
> We compare the **full model** (`dep_delay + air_time + distance`) to a **reduced model** (`dep_delay + air_time`) using a partial F-test. This tells us whether the added predictor provides significant additional explanatory power.
